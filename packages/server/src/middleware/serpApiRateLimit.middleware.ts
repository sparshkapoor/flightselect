...
function getClientKey(ip: string, prefix: string): string {
  return `${prefix}:${ip}`;
}

export function createSerpApiRateLimit(clientWindowSeconds: number, keyPrefix = 'serpapi:client') {
  return async function serpApiRateLimitMiddleware(req: Request, res: Response, next: NextFunction): Promise<void> {
    const clientIp = req.ip || req.socket.remoteAddress || 'unknown';

    try {
      // --- Layer 1: Per-client sliding window ---
      const clientKey = getClientKey(clientIp, keyPrefix);
      ...
      await redis.set(clientKey, '1', 'EX', clientWindowSeconds);
      ...
      res.setHeader('X-RateLimit-Client-RetryAfter', String(clientWindowSeconds));

      logger.info({ clientIp, globalCount: newCount, remaining: MONTHLY_LIMIT - newCount }, 'SerpAPI search token consumed');

      next();
    } catch (err) {
      // Fail closed: this middleware exists to protect a metered, paid SerpAPI
      // quota. Letting requests through on a Redis hiccup would silently
      // remove the one thing standing between this app and an overage bill.
      logger.error({ err }, 'SerpAPI rate limiter error — failing closed (limiter unavailable)');
      res.status(503).json({
        status: 'error',
        message: 'Rate limiter temporarily unavailable. Please try again shortly.',
        limitType: 'limiter_unavailable',
      });
    }
  };
}

export const serpApiRateLimit = createSerpApiRateLimit(60);
